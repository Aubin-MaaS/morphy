...
echo -e ""
echo -e "2. ${YELLOW}To merge changes to main and create tag:${NC}"
echo -e "   git checkout main && git merge $(git branch --show-current) && git tag v$(grep '^version:' zikzak_morphy/pubspec.yaml | sed 's/version: //') && git push origin main --tags"
echo -e ""
